...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  default_node_pool {
    ...
    temporary_name_for_rotation  = substr("${var.default_node_pool_name}rotation", 0, 12)
    only_critical_addons_enabled = var.only_critical_addons_enabled
    vnet_subnet_id               = var.vnet_subnet_id
    pod_subnet_id                = var.pod_subnet_id
    zones                        = var.default_node_pool_availability_zones
    node_labels                  = var.default_node_pool_node_labels
    auto_scaling_enabled         = var.default_node_pool_node_auto_scaling_enabled
    host_encryption_enabled      = var.default_node_pool_node_host_encryption_enabled
    node_public_ip_enabled       = var.default_node_pool_node_node_public_ip_enabled
    max_pods                     = var.default_node_pool_node_max_pods
    max_count                    = var.default_node_pool_node_max_count
    min_count                    = var.default_node_pool_node_min_count
    node_count                   = var.default_node_pool_node_count
    os_disk_type                 = var.default_node_pool_node
    tags                         = var.tags
